...
// Implemented and modified for use as XeBRA PreCluster Algorithm by. F.Kuger (Mai 2019)
...
namespace clustering
{
	...
	public:
		...
		// 'eps' is the search space for neighbors in the range [0,eps] mm. 
...
#pragma omp parallel for
				for (int j = i; j < (int)cl_d.rows(); ++j)
				{
					...
					if (i != j)
					{
						...
						{
							double e = diff[k];
							d_m(i, j) += fabs(e)*fabs(e)*W[k];
						}
						d_m(i,j) = std::sqrt(d_m(i,j));
